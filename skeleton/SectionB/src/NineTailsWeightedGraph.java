...
public class NineTailsWeightedGraph {
  ...
  private void constructGraph() {
    ...
    for (int i = 1; i <= NUM_CONFIGURATIONS; i++) {
      configurations.add(i, new PriorityQueue<WeightedEdge>());
    }
    for (int i = 1; i <= NUM_CONFIGURATIONS; i++) {
      char[] conf = indexToConfiguration(i);
      for (int j = 0; j < 9; j++) {
        if (conf[j] == 'H') {
          FlipResult flipResult = flipConfiguration(i, j);
          configurations.get(flipResult.newIndex).add(new WeightedEdge(i,
              flipResult.newIndex, flipResult.numFlips));
        }
      }
    }
    boolean[] contained = new boolean[NUM_CONFIGURATIONS];
    for (int i = 1; i <= NUM_CONFIGURATIONS; i++) {
      Iterator it = configurations.get(i).iterator();
      while (it.hasNext()) {
        WeightedEdge edge = (WeightedEdge) it.next();
        contained[edge.parent - 1] = true;
      }
    }
  ...
  private void constructMinimumSpanningTree() {
    ...
    while (visited.size() < confCopy.size()) {
      for (int i = 1; i <= visited.size(); i++) {
        int childIndex = visited.get(i);
        // Add its parents to visited
        Iterator it = confCopy.get(childIndex).iterator();
        while (it.hasNext()) {
          WeightedEdge edge = (WeightedEdge) it.next();
          // The parent we want to add
          int parentIndex = edge.parent;
          // See if it's already visited
          if (!visited.contains(parentIndex)) {
            visited.add(visited.size() + 1, parentIndex);
            System.out.println(parentIndex);
            // Store the current minimum cost
            int currMinCost = costs[childIndex] + edge.weight;
            // See if there is a lower cost to go to this parent
            for (int j = 1; j <= visited.size(); j++) {
              if (j != i) {
                int candIndex = visited.get(j);
                Iterator it2 = confCopy.get(candIndex).iterator();
                while (it2.hasNext()) {
                  WeightedEdge candEdge = (WeightedEdge) it2.next();
                  if (candEdge.parent == parentIndex) {
                      if (costs[candIndex] + candEdge.weight < currMinCost) {
                        currMinCost = costs[candIndex] + candEdge.weight;
                        childIndex = candEdge.child;
                    }
                  }
                }
              }
            }
            // Update Costs and nextMoves
            costs[parentIndex] = currMinCost;
            nextMoves[parentIndex] = childIndex;
          }
        }
      }
    }
